...
class User:
    def __init__(self, user_config: dict, username: str, chat_users: list[str] = None):
        ...
        self.chat_users = [user.split("-")[0] for user in chat_users] if chat_users is not None else []
        ...
        self.base_prompt = user_config["generation"]["prompt"]
        users = ", ".join(self.chat_users[:-1]) + " and " + self.chat_users[-1] if len(self.chat_users) > 1 else (self.chat_users[0] if len(self.chat_users) == 1 else "no one")
        self.system_prompt = j2.Template(self.base_prompt).render(username=self.username, users=users)
    ...
    def decide_message(self) -> str:
        ...
        if self.llm_enabled:
            ...
            if len(clean_history) == 0:
                clean_history = "The conversation hasn't started yet..."
            final_messages = [
                {
                    "role": "system",
